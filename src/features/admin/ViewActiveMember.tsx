...
const ViewActiveMember = ({closeModal, approveApplication, application, user}: AppProps) => {
  ...
  return (
    <div className="relative">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 my-2">
            ...
            <div className="">
              ...
              <p className="text-[#272727] text-[16px] font-[400] capitalize">$ {application.amount}</p>
            </div>
          </div>
          <div className="flex space-x-16 items-center text-center mt-7 mb-12">
            ...
            })} onClick={onApprove} disabled={showSuccModal}>Activate</button>
            <button className={classNames("font-[700] text-[15px] border-2 border-green2 text-green2  px-6 py-2 rounded-md", {
              "hover:shadow-xl": !showSuccModal,
              "hover:cursor-not-allowed": showSuccModal,
            })} onClick={onApprove} disabled={showSuccModal}>Deactivate</button>
          </div>
        </div> 
      </div>
      {/* SUCCESS MODAL */}
      {showSuccModal && <div className="text-[#307D0B] w-[400px] rounded-lg absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 shadow-xl text-center bg-slate-100 py-6">
          ...
          <p className="">This member has been sucessuflly activated</p>
        </div>
      }
    </div>
  )
...
export default ViewActiveMember;