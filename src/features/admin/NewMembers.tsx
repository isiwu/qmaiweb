...
import { approveApplication, getNewMembers, MemberData, memberData, viewMember } from "./adminSlice";
...
const NewMembers = () => {
  const newMembers = useAppSelector(state => state.admin.members);
  ...
  const handleApproveApplication = async (id: string) => {
    ...
    if (!requestError) {
      await dispatch(getNewMembers()).unwrap();
      return true;
    }
  ...
  return (
    <div>
      {newMembers?.length? <div className="border w-[100%] py-8 rounded-lg my-8">
      ...
      </div>: <div className="text-center">NO NEW MEMBER APPLICATION</div>}
      {/* LOADER */}
      {
