...
//import { setMakePayment } from "../../features/memberships/membershipSlice";
...
const MembershipPrincipalForm = ({applicationType, formData, handleChange, handleChangeSelect, handleClick}: AppProps) => {
      ...
      <div>
        ...
        <div className="flex items-center my-8 space-x-4 px-4">
          <input type="radio" className={classNames("hover:cursor-pointer checked:bg-green3 checked:outline checked:outline-offset-2 checked:w-5 checked:h-5 checked:outline-green6 checked:rounded-full checked:appearance-none", {
            "w-7 h-7": formData.membershipType === "graduate auditor membership",
            "w-16 h-16": formData.membershipType !== "graduate auditor membership"
          })} id="studying" onChange={handleChange("studyingQMAI")} />
          {formData.membershipType === "associate auditor membership" && <label htmlFor="studying" className="font-[10] leading-[18px] text-[#676767] hover:cursor-pointer text-[18px]">Applicant must have passed a QMSAI-certified Internal Auditor or QMSAI recognized Internal Auditor training course for any of the ISO Management System. Or Passed a QMSAI certified Lead Auditor course or QMSAI recognized Lead Auditor training course for any of the ISO Management System.</label>}
          {formData.membershipType === "principal auditor membership" && <label htmlFor="studying" className="font-[10] leading-[18px] text-[#676767] hover:cursor-pointer text-[18px]">Are you a lead audits or are you and audit team leader conducting full first party audit (internal Audit) management system, or full second-party (supplier audits), or full third-party management system audits? Then this grade is for you.</label>}
          {formData.membershipType === "charttered auditor membership" && <label htmlFor="studying" className="font-[10] leading-[18px] text-[#676767] hover:cursor-pointer text-[18px]">Are you a lead audits or are you and audit team leader conducting full first party audit (internal Audit) management system, or full second-party (supplier audits), or full third-party management system audits? Then this grade is for you.</label>}
          {formData.membershipType === "full membership" && <label htmlFor="studying" className="font-[10] leading-[18px] text-[#676767] hover:cursor-pointer text-[18px]">Are you a lead audits or are you and audit team leader conducting full first party audit (internal Audit) management system, or full second-party (supplier audits), or full third-party management system audits? Then this grade is for you.</label>}
        </div>
        ...
        {formData.membershipType === "associate auditor membership" && <div className="flex items-center space-x-4 my-5 px-4">
          <div><RiErrorWarningFill /></div>
          <div className="text-[#CCCCCC] text-[14px]">Aleast 4 years work experience  and 2 years system  experience</div>
        </div>}
        {formData.membershipType === "principal auditor membership" && <div className="flex items-center space-x-4 my-5 px-4">
          <div><RiErrorWarningFill /></div>
          <div className="text-[#CCCCCC] text-[14px]">Aleast 5years work experience  and 3 years system experience</div>
        </div>}
        {formData.membershipType === "chartterred auditor membership" && <div className="flex items-center space-x-4 my-5 px-4">
          <div><RiErrorWarningFill /></div>
          <div className="text-[#CCCCCC] text-[14px]">Aleast 10 years work experience  and 6 years system experience</div>
        </div>}
        {formData.membershipType === "full membership" && <div className="flex items-center space-x-4 my-5 px-4">
          <div><RiErrorWarningFill /></div>
          <div className="text-[#CCCCCC] text-[14px]">Aleast 5years work experience  and 3 years system experience</div>
        </div>}
      </div>
...
export default MembershipPrincipalForm;