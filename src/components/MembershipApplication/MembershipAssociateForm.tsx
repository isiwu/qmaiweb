...
const AssociateForm = ({applicationType, formData, handleChange, handleChangeSelect, handleClick}: AppProps) => {
  ...
  return (
    <div>
      <div>
        ...
        <hr />
        <div className="flex items-center my-8 space-x-6 px-4">
          <input type="radio" className={classNames("w-16 h-16 hover:cursor-pointer checked:bg-green3 checked:outline checked:outline-offset-2 checked:w-5 checked:h-5 checked:outline-green6 checked:rounded-full checked:appearance-none")} id="studying" onChange={handleChange("studyingQMAI")} />
      ...
      <div className="flex justify-center items-center mt-16 px-4">
        <ApplicationButton text={"Contine"} handleClick={handleContinue} disabled={showModal?true:membershipStatus === "loading"?true:false} />
      </div>
    </div>
  )
}
